...
var require_color_classifier = __commonJS({
    "node_modules/color-classifier/color-classifier.js"(exports, module2) {
        (function (global, factory) {
            ...
            var W3C = ["#000000", "#EDEDED", "#c0c0c0", "#ffffff", "#800000", "#ff0000", "#008000", "#00ff00", "#808000", "#ffff00", "#008080", "#00ffff", "#000080", "#0000ff", "#800080", "#ff00ff"];
            var RAINBOW = ["#000000", "#EDEDED", "#ffffff", "#ff0000", "#ffa500", "#ffff00", "#008000", "#00ffff", "#0000ff", "#800080"];
...
var DEFAULT_SETTINGS = {
    ...
    isDoubleSpaced: true,
    highlightExportTemplate: "{{highlight}}\n - {{comment}}\n - {{tag}}",
...
// This function replaces all templates in a given note string with their corresponding values.
// entriesArray: an array of all the keys in the selectedEntry object
// note: the original note string
// selectedEntry: the selected entry object containing all the metadata
// returns: the updated note string with all the templates replaced with their values
function replaceAllTemplates(entriesArray, note, selectedEntry) {
    ...
    for (let z = 0; z < entriesArray.length; z++) {
        ...
        // create a template with the key
        const KW_Brackets = "{{" + KW + "}}";
        // replace the template with the value
        copy = replaceTemplate(copy, KW_Brackets, `${selectedEntry[KW]}`);
        // print the key being replaced
        console.log("Replacing template:", KW);
    }
    return copy; // return the updated note
}

// This function escapes special characters in a string so it can be used as a regular expression pattern.
// stringAdd: the string to be escaped
// returns: the escaped string
function escapeRegExp(stringAdd) {
    return stringAdd.replace(/[.*+\-?^${}()|[\]\\]/g, "\\$&");
}

// This function replaces all instances of a substring in a string with a new string.
// stringAdd: the original string
// find: the substring to find
// replace: the new string to replace the substring with
// returns: the updated string with all instances of the substring replaced
function replaceTemplate(stringAdd, find, replace) {
    return stringAdd.replace(new RegExp(escapeRegExp(find), "g"), replace);
}


// make a string into a wiki link, a quote or a tag
var makeWiki = (str) => "[[" + str + "]]";
var makeQuotes = (str) => '"' + str + '"';
var makeTags = (str) => "#" + str;

// This function creates an author or editor key based on the given creators array.
// The author key contains the names of the authors formatted according to APA style.
// The editor key contains the names of the editors formatted according to APA style.
var createAuthorKey = (creators) => {
    ...
    // Loop through each creator in the array
    for (let creatorindex = 0; creatorindex < creators.length; creatorindex++) {
        const creator = creators[creatorindex];

        // Check if the creator is an author or an editor and add their name to the appropriate array
        if (creator.creatorType === "author") {
            if (creator.hasOwnProperty("name")) {
                authorKey.push(creator.name);
            } else if (creator.hasOwnProperty("lastName")) {
    ...
    // Format the author key based on the number of authors
    ...
    // Return the author key if it has any values
    if (authorKey.length > 0) {
        return authorKeyFixed;
    }

    // Format the editor key based on the number of editors
    ...
    // Return the editor key if it has any values
...
// This function takes an array of creator objects as input and returns a formatted author key
var createAuthorKeyFullName = (creators) => {
    // Create arrays to hold author and editor names and their reverse versions (last name first)
    const authorKey = [];
    const authorKeyReverse = [];
    const editorKey = [];
    const editorKeyReverse = [];
    // Initialize variables for the final author and editor keys
    let authorKeyFixed = "";
    let editorKeyFixed = "";
    // Loop through each creator in the array
    for (let creatorindex = 0; creatorindex < creators.length; creatorindex++) {
        const creator = creators[creatorindex];
        // Check if creator is an author
        if (creator.creatorType === "author") {
            // Check if creator has a name property, if so, add to author key array
            if (creator.hasOwnProperty("name")) {
                authorKey.push(creator.name);
                // Check if creator has both a first name and last name, if so, add to author key and reverse author key arrays
            } else if (creator.hasOwnProperty("lastName") && creator.hasOwnProperty("firstName")) {
                authorKey.push(creator.lastName + ", " + creator.firstName);
                authorKeyReverse.push(creator.firstName + " " + creator.lastName);
                // Check if creator only has a last name property, if so, add to author key array
            } else if (creator.hasOwnProperty("lastName") && !creator.hasOwnProperty("firstName")) {
                authorKey.push(creator.lastName);
                // Check if creator only has a first name property, if so, add to author key array
            } else if (!creator.hasOwnProperty("lastName") && creator.hasOwnProperty("firstName")) {
                authorKey.push(creator.firstName);
            }
            // Check if creator is an editor (same logic as author)
        } else if (creator.creatorType === "editor") {
            if (creator.hasOwnProperty("name")) {
                editorKey.push(creator.name);
            } else if (creator.hasOwnProperty("lastName") && creator.hasOwnProperty("firstName")) {
    ...
    // Format the final author key based on the number of authors
    if (authorKey.length == 1) {
        authorKeyFixed = authorKey[0];
    }
    if (authorKey.length == 2) {
    ...
    // Return the author key if there are any authors, otherwise check editor key
    if (authorKey.length > 0) {
        return authorKeyFixed;
    }
    // Format the final editor key based on the number of editors (same logic as author)
    if (editorKey.length == 1) {
        editorKeyFixed = editorKey[0];
    }
    if (editorKey.length == 2) {
...
/**
 * Replaces tag-related placeholders in the metadata with the appropriate values.
 * @param {object} selectedEntry - The selected entry object from Zotero.
 * @param {array} arrayExtractedKeywords - The list of keywords extracted from the PDF.
 * @param {string} metadata - The metadata string to replace placeholders in.
 * @param {string} divider - The divider to use between tags when multiple tags are present.
 * @returns {string} The updated metadata string.
 */
function replaceTagList(selectedEntry, arrayExtractedKeywords, metadata, divider) {
    // Initialize an array to hold the Zotero tags
    selectedEntry.zoteroTags = [];

    // If there are any tags in the selected entry, add them to the zoteroTags array
    ...
    // If the divider doesn't end with a space, add a space to the end
    if (divider.slice(-1) !== " ") {
        divider = divider + " ";
    }

    // Sort the Zotero and PDF tags separately and combine them
    ...
    // Replace Zotero tag placeholders in the metadata with the appropriate values
    ...
    // Replace PDF tag placeholders in the metadata with the appropriate values
    if (tagsPDF.length > 0) {
    ...
    // Replace combined tag placeholders in the metadata with the appropriate values
...
// Define a new class for updating the Zotero library, which extends the Modal class from the Obsidian API
var updateLibrary = class extends import_obsidian2.Modal {
    ...
    // Method that gets called when the modal is opened
    onOpen() {
        // Log a message to the console
        console.log("Updating Zotero library");
        // Get the path to the JSON file containing the BetterBibTeX data
        const jsonPath = this.app.vault.adapter.getBasePath() + "/" + this.plugin.settings.bibPath;
        // If the file doesn't exist, show a warning message and return
        if (!fs.existsSync(jsonPath)) {
            new import_obsidian2.Notice("No BetterBibTex Json file found at " + jsonPath);
        }

        const rawdata = fs.readFileSync(this.app.vault.adapter.getBasePath() + "/" + this.plugin.settings.bibPath); // Read the contents of the JSON file
        // Parse the JSON data into a JavaScript object
        const data = JSON.parse(rawdata.toString());
        // Create an array to store the citation keys of the updated entries
        const bibtexArray = [];
        // Get the date of the last update from the plugin settings
        ...
        for (let index = 0; index < data.items.length; index++) {
            ...
            const selectedEntry = data.items[index];
            // Create an object to store the citation key and note date of the current item
            const bibtexArrayItem = {};

            // If the current item doesn't have a citation key, skip it
            if (selectedEntry.hasOwnProperty("citationKey") == false)
                continue;

            // Store the citation key of the current item
            bibtexArrayItem.citationKey = selectedEntry.citationKey;

            // Create an array to store the dates of the notes
            const noteDateModifiedArray = [];
            noteDateModifiedArray.push(selectedEntry.dateModified);

            // Loop through each note of the current item and add its date to the array
            ...
            // Get the date of the most recently modified note
            const datemodified = new Date(noteDateModifiedArray[0]);

            // If the most recently modified note is older than the last update, skip the current item
            if (datemodified < lastUpdate)
                continue;

            // If the setting is to only update existing notes, and the note for the current item doesn't exist, skip the current item
            ...
            // Create a new note for the current item
            this.plugin.createNote(selectedEntry, data);

            // Add the citation key to the array of updated entries
            bibtexArray.push(selectedEntry.citationKey);
        }

        // Show a message indicating the number of
...
var MyPlugin = class extends import_obsidian6.Plugin {
    ...
    /**
     * Parses the metadata of a selected entry using a given template.
     * @param {Object} selectedEntry - The selected entry.
     * @param {string} templateOriginal - The original template to use.
    ...
    parseMetadata(selectedEntry, templateOriginal) {
        ...
        // Initialize the note string with the template
        let note = template;

        // Add the author and editor lists to the note
        ...
        // Extract the year from the date field and add it to the selectedEntry object
        if (selectedEntry.hasOwnProperty("date")) {
            selectedEntry.year = selectedEntry.date.match(/\d\d\d\d/gm) + "";
        }
        // Add the Zotero library link to the selectedEntry object
        ...
        // Rename the citationKey field to citeKey
        selectedEntry.citeKey = selectedEntry.citationKey;

        // Normalize the item type names
        ...
        // Capitalize the first letter of the item type name
        selectedEntry.itemType = selectedEntry.itemType.charAt(0).toUpperCase() + selectedEntry.itemType.slice(1);

        // Generate the citation strings for the selectedEntry object
        ...
        // Generate the citation strings for a journal article
    ...
    parseCollection(selectedEntry, data, metadata) {
        ...
        // Fill collection array with names and get colllection parent codes
    ...
    createNote(selectedEntry, data) {
        // Extract author information from the selected entry
        ...
        bugout.log("+++++++++++++++++++++++++++++++++++++++++++")
        bugout.log(selectedEntry.authorKey)
        bugout.log("+++++++++++++++++++++++++++++++++++++++++++++++++++++++")

        // Import the note template
        const templateNote = this.importTemplate();
        // bugout.log("Template: \n" + templateNote);

        // Parse metadata from the selected entry into the note template
        let litnote = this.parseMetadata(selectedEntry, templateNote);
        bugout.log("=======================================================")
        bugout.log(selectedEntry);

        // Parse collection information from the data object into the note
        litnote = this.parseCollection(selectedEntry, data, litnote);
        bugout.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        bugout.log("first" + litnote)

        // Extract annotations and images from the selected entry
        ...
        // Replace placeholders in the note template with extracted annotations, images, and other information
        const placeholdersMapping = {
            "{{PDFNotes}}": resultAnnotations.extractedAnnotations,
            "{{UserNotes}}": resultAnnotations.extractedUserNote,
            "{{Yellow}}": resultAnnotations.extractedAnnotationsYellow,
            "{{Red}}": resultAnnotations.extractedAnnotationsRed,
            "{{Green}}": resultAnnotations.extractedAnnotationsGreen,
            "{{Blue}}": resultAnnotations.extractedAnnotationsBlue,
            "{{Purple}}": resultAnnotations.extractedAnnotationsPurple,
            "{{Black}}": resultAnnotations.extractedAnnotationsBlack,
            "{{White}}": resultAnnotations.extractedAnnotationsWhite,
            "{{Gray}}": resultAnnotations.extractedAnnotationsGray,
            "{{Cyan}}": resultAnnotations.extractedAnnotationsCyan,
            "{{Orange}}": resultAnnotations.extractedAnnotationsOrange,
            "{{Magenta}}": resultAnnotations.extractedAnnotationsMagenta,
            "{{CustomHex}}": resultAnnotations.extractedAnnotationsCustomHex,
            "{{Images}}": resultAnnotations.extractedImages
        };

        for (const placeholder in placeholdersMapping) {
            litnote = litnote.replace(placeholder, placeholdersMapping[placeholder]);
        }

        // Replace tags and missing fields in the note template
        let extractedKeywords = resultAnnotations.extractedKeywords;
        if (extractedKeywords == void 0) {
            extractedKeywords = [];
        }
        bugout.log("############################################################")
        bugout.log("before keywordes" + litnote)
        bugout.log("############################################################")
        bugout.log("selected entry" + selectedEntry.tags)
        bugout.log("\n")
        bugout.log("extracted keywords" + extractedKeywords)
        bugout.log("\n")
        bugout.log(this.settings.multipleFieldsDivider)
        bugout.log("\n\n")
        litnote = replaceTagList(selectedEntry, extractedKeywords, litnote, this.settings.multipleFieldsDivider);
        bugout.log("after keywords" + litnote)
        bugout.log("############################################################")
        ...
        // Compare old and new notes if setting is not "Overwrite Entire Note"
        ...
        // Save the note to a file and display a success message
        bugout.log("=======================================================")
