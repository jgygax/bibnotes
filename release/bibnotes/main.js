					...
					},
					{
						...
						value: function classifyFromArray(colors) {
							...
							array.forEach(function (obj) {
								...
								var color = obj.color;
								var _results$filter = results.filter(function (
									o
								) {
...
var Debugout = (function () {
	...
	return (
		...
		(t.prototype.formatDate = function (t2) {
			return (
				t2 === void 0 && (t2 = new Date()), "[" + t2.toISOString() + "]"
			);
		}),
...
var MyPlugin = class extends import_obsidian6.Plugin {
	...
	formatNoteElements(noteElements, citeKey) {
		...
		for (let i = 0; i < noteElements.length; i++) {
			...
			const commentFormatBeforeColoured = commentFormatBefore.replace(
				"SELECTED_COLOUR",
				lineElements.highlightColour + ";"
			);
			const tagFormatBeforeColoured = tagFormatBefore.replace(
				"SELECTED_COLOUR",
				lineElements.highlightColour + ";"
			);
			...
			const TempTag = lineElements.inlineTagsArray.map(
				(i2) =>
					tagPrepend + tagFormatBeforeColoured + i2 + tagFormatAfter
			);
			...
			const TempTagNoPrepend = lineElements.inlineTagsArray.map(
				(i2) => tagFormatBeforeColoured + i2 + tagFormatAfter
			);
	...
	createNote(selectedEntry, data) {
		...
		for (const placeholder in placeholdersMapping) {
			litnote = litnote.replace(
				placeholder,
				placeholdersMapping[placeholder]
			);
		}
